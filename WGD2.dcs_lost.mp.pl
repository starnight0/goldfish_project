...
if (0 && $can_use_threads) {
	my $thi=0;
	for ($k=0; $k<3; $k++) {
		$na[$k] = @{$aligns[$k]};
		$trees[$k][$i] = shared_clone({});
		for ($i=0; $i<2; $i++) {
			$threads[$thi] = threads->create( sub {
					print STDERR "Build Interval Tree of Alignment $k $i\n";
					my $tree = align2::build_interval_tree_for_liftover_m2($aligns[$k], $i);
					print STDERR "Finish Interval Tree of Alignment $k $i\n";
					{ lock(@trees); $trees[$k][$i] = shared_clone $tree;  }
	...
	for (my $k=0; $k<3; $k++) {
		for (my $i=0; $i<2; $i++) {
			...
			my $v3;
			if ($k==2 && $i==0) {
				$v3 = $trees[$k][$i]{LG26}->fetch(23757279, 23757823);
			}
			$thi++;
		}
	}
...
if (0 && $can_use_threads) {
	...
	for ($thi=0; $thi<$thn; $thi++) { $threads[$thi]->join(); }
	$thn=0;
	...
	foreach my $name (keys(%{$exons[1]})) {
		...
		$threads[$thi] = threads->create( sub {
				$map_exons[4+$k]{$name} = align2::liftover_m2($exons[1]{$name}, $aligns[2], $k, $trees[2][$k]); } );
	}
	}
